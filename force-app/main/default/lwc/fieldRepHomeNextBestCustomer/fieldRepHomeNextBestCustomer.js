    ...
    async init() {
        ...
        try {
            if (isOfflineMode()) {
                ...
                if (cached?.nbcRows?.length) {
                    this.rows = cached.nbcRows;
                    return;
                }
                this.rows = [];
                return;
            ...
            const existing = (await getHomeMetricsCache(getUserHomeMetricsKey(Id))) || {};
            await putHomeMetrics(getUserHomeMetricsKey(Id), { ...existing, nbcRows: this.rows });
        } catch (e) {
            const cached = await getHomeMetricsCache(getUserHomeMetricsKey(Id));
            if (cached?.nbcRows?.length) {
                this.rows = cached.nbcRows;
