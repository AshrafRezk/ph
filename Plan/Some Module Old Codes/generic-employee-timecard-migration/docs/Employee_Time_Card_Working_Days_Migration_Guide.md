...
**Generic package note:** This bundle (`generic-employee-timecard-migration`) is the IDH-free edition. It excludes Al Mokhtabar/Al Borg/Borg Scan fields, `AssignedUserTargetRevenue__c`, commission logic, and the Visits report folder copy. It adds a walk-in-only pace dashboard LWC and `Employee_Time_Card_Recalculate_Walk_in_Achievements` flow. Deploy with `manifest/package.xml`.
...
### Permission sets (included in generic package)
- `Employee_Time_Card_Module` — time card objects, flows, reports, walk-in fields
- `Employee_Pace_Dashboard` — pace LWC, team grid, CSV export (`View_Employee_Pace_Team` custom permission)

### Pace dashboard (generic package only)
- LWC: `employeePaceDashboard` — walk-in revenue and visits vs targets
- Flow: `Employee_Time_Card_Recalculate_Walk_in_Achievements`
- Apex: `EmployeeTimeCardAchievementService`, `EmployeeTimeCardPaceService`, export controllers

Note:
- Populate `Walk_in_Revenue__c`, `Walk_in_Visits__c`, and target fields on `Employee_Time_Card__c` in the target org (no AUTR in this package).
- Add `employeePaceDashboard` to a Home page or Tab after deploy.

### Permission sets (legacy IDH org — not in generic package)
